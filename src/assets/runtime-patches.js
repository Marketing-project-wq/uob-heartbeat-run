// Runtime patches carried over from the old bundler wrapper so the un-bundled
// build behaves identically. These are DOM tweaks applied after the app mounts.
// (Long term: fold each of these into the app source and delete this file.)
document.addEventListener('DOMContentLoaded', function () {
    ...
    var fixEmployeeId = function() {
      document.querySelectorAll('label, div, span').forEach(function(el) {
        if (el.textContent && el.textContent.includes('EMPLOYEE NIK')) {
          el.textContent = el.textContent.replace('EMPLOYEE NIK', 'EMPLOYEE ID');
        }
      });
    ...
    window.addEventListener('error', function(e) {
      if (e.message && (e.message.includes('log') || e.message.includes('run') || e.message.includes('activity'))) {
        console.warn('[Log Run Error]:', e.message);
